...
def updateData():
    send = [0x00, 0x00, pwm1, 0x00, pwm2, contagem, 0x00, 0x00, 0x00] # este é o dado a ser enviado(line d0-d7)
...
try:
    ...
    while pwm1<max1 or pwm2<max2:
        pwm1 = pwm1+1
        if pwm1>max1: pwm1 = max1
        pwm2 = pwm2+1
        if pwm2>max2: pwm2 = max2
        updateData()
    ...
    while pwm1>0x00 or pwm2>0x00:
        pwm1 = pwm1-1
        if pwm1<0x00: pwm1 = 0x00
        pwm2 = pwm2-1
        if pwm2<0x00: pwm2 = 0x00
        updateData()
