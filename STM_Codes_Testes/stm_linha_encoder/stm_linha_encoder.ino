 ...
 * Description: Código para calibrar a resolução do encoder.
 */
...
volatile uint64_t count_encoder1 = 0;
volatile uint64_t count_encoder2 = 0;
unsigned long int last_time_count_encoder = 0;

byte dataIn[LINES_IN][8] = { // matriz de entrada
  {0, 0, 0, 0, 0, 0, 0, 0} //pwm1, pwm2, pwm3, pwm4, count
};

byte dataOut[LINES_OUT][8] = { // matriz de saida
  {0, 0, 0, 0, 0, 0, 0, 0}, // linha
  {0, 0, 0, 0, 0, 0, 0, 0}, // mt1
  {0, 0, 0, 0, 0, 0, 0, 0}  // mt2
};
...
void loop() {
  ...
  // data encoder1
  dataOut[1][0] = (uint8_t)((count_encoder1 & 0xFF00000000000000) >> 56);
  dataOut[1][1] = (uint8_t)((count_encoder1 & 0x00FF000000000000) >> 48);
  dataOut[1][2] = (uint8_t)((count_encoder1 & 0x0000FF0000000000) >> 40);
  dataOut[1][3] = (uint8_t)((count_encoder1 & 0x000000FF00000000) >> 32);
  dataOut[1][4] = (uint8_t)((count_encoder1 & 0x00000000FF000000) >> 24);
  dataOut[1][5] = (uint8_t)((count_encoder1 & 0x0000000000FF0000) >> 16);
  dataOut[1][6] = (uint8_t)((count_encoder1 & 0x000000000000FF00) >> 8);
  dataOut[1][7] = (uint8_t)((count_encoder1 & 0x00000000000000FF));
  // data encoder2
  dataOut[2][0] = (uint8_t)((count_encoder2 & 0xFF00000000000000) >> 56);
  dataOut[2][1] = (uint8_t)((count_encoder2 & 0x00FF000000000000) >> 48);
  dataOut[2][2] = (uint8_t)((count_encoder2 & 0x0000FF0000000000) >> 40);
  dataOut[2][3] = (uint8_t)((count_encoder2 & 0x000000FF00000000) >> 32);
  dataOut[2][4] = (uint8_t)((count_encoder2 & 0x00000000FF000000) >> 24);
  dataOut[2][5] = (uint8_t)((count_encoder2 & 0x0000000000FF0000) >> 16);
  dataOut[2][6] = (uint8_t)((count_encoder2 & 0x000000000000FF00) >> 8);
  dataOut[2][7] = (uint8_t)((count_encoder2 & 0x00000000000000FF));
  ...
  if(readFlag){ // ler a data recebida
    ...
    for(byte i = 0; i < qt_bits; i++){ // ler os bytes 
      ...
      delayMicroseconds(75); // espera para que o buffer interno possa preparar o próximo byte para a leitura (9 bytes 10us cada - 90us ~ 0.1ms)
    }
